...
The categorical variables (Gender, Day, & Appointment Month) are converted into integer variables by coding them using dummies. The target variable (Status) is left as a categorical variable since the goal is to predict the class of a categorical variable (Status). 
...
This classifier has an overall accuracy rate of 69.66%, sensitivity rate of 1.8%, and a specificity rate of 99%. The selected features improved the accuracy rate and the specificity rate from the ANN model but not sensitivity rate. 
...
Analytics Vidhya (2016, March 28). Practical Guide to deal with Imbalanced Classification Problems in R. Retrieved from https://www.analyticsvidhya.com/blog/2016/03/practical-guide-deal-imbalanced-classification-problems/
